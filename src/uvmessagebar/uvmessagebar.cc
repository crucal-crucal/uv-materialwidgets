...
#include "uvawesomebutton/uvawesomebutton.hpp"
...
void CUVMessageBarPrivate::onCloseButtonClicked() {
	Q_Q(CUVMessageBar);
...
void CUVMessageBarPrivate::onOtherMessageBarClosed(const UVMessageBarType::PositionPolicy positionPolicy, const int barIndex) {
...
: QWidget(parent), d_ptr(new CUVMessageBarPrivate(this)) {
	...
	d->closeButton = new CUVAwesomeButton(CUVIconType::Xmark, 17, d->closeButtonWidth, 30, this);
	...
	connect(d->closeButton, &CUVAwesomeButton::clicked, d, &CUVMessageBarPrivate::onCloseButtonClicked);
