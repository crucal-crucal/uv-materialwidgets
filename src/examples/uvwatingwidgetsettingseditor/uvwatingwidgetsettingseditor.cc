...
CUVWatingWidgetSettingsEditor::CUVWatingWidgetSettingsEditor(QWidget* parent): QWidget(parent), m_pWating(new CUVMaterialWatingWidget) {
	createCtrl();
	customLayout();
	initData();
	setupForm();
...
void CUVWatingWidgetSettingsEditor::initConnection() {
	...
	connect(m_pBtnStart, &QPushButton::clicked, m_pWating, &CUVMaterialWatingWidget::start);
	connect(m_pBtnStop, &QPushButton::clicked, m_pWating, &CUVMaterialWatingWidget::stop);
}

void CUVWatingWidgetSettingsEditor::setupForm() const {
	switch (m_pWating->textPosition()) {
		case CUVMaterialWatingWidget::Bottom: {
			m_pCbTextPosition->setCurrentIndex(0);
			break;
		}
		case CUVMaterialWatingWidget::Top: {
...
void CUVWatingWidgetSettingsEditor::updateWidget() const {
	switch (m_pCbTextPosition->currentIndex()) {
		case 0: {
			m_pWating->setTextPosition(CUVMaterialWatingWidget::Bottom);
			break;
		}
		case 1: {
			m_pWating->setTextPosition(CUVMaterialWatingWidget::Top);
			break;
		}
		default: break;
	}
