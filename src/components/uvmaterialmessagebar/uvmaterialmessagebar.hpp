...
class CUVMATERIALMESSAGEBAR_EXPORT CUVMaterialMessageBar final : public QWidget {
	Q_OBJECT
	Q_DISABLE_COPY(CUVMaterialMessageBar)
	Q_DECLARE_PRIVATE(CUVMaterialMessageBar)

public:
	explicit CUVMaterialMessageBar(const UVMessageBarType::PositionPolicy& policy, const UVMessageBarType::MessageMode& messageMode,
	                               const QString& title, const QString& message, int displayMsec, QWidget* parent = nullptr);
	~CUVMaterialMessageBar() override;
...
protected:
	...
	const QScopedPointer<CUVMaterialMessageBarPrivate> d_ptr{ nullptr };
};
