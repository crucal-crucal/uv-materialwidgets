...
void CUVMessageBtnSettingsEditor::createCtrl() {
	m_pBtnSuccess = new CUVMessageButton("Success", this);
	m_pBtnSuccess->setBarTitle("Success");
	m_pBtnSuccess->setBarText("Successfully message ...");

	m_pBtnError = new CUVMessageButton("Error", UVMessageBarType::Error, this);
	...
	m_pBtnWaring = new CUVMessageButton("Waring", UVMessageBarType::Warning, this);
	...
	m_pBtnInfo = new CUVMessageButton("Infomation", UVMessageBarType::Info, this);
	...
	connect(m_pBtnTest, &QPushButton::clicked, this, [this]() {
		CUVMessageBar::success("test test test test", "test message ...", 2000, UVMessageBarType::PositionPolicy::Top, window());
	});
